...
bool LaserMapping::Run() {
    ...
    /// 更新kf_for_imu
    {
        ...
        if (!measures_.imu_.empty()) {
            // kf_ has already been propagated to the lidar frame end time.
            // Replaying from the last raw IMU timestamp introduces a small
            // positive offset that disappears at the next lidar correction,
            // making the published DR timestamp move backwards.
            double t = kf_.GetX().timestamp_;
